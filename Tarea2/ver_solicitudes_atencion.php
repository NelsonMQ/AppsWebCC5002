...
$numberOfPages = numberOfPagesSolicitudes($db);

$solicitudes_data = getSolicitudesData($db, $offset);
    ...
    <body>
        ...
        <div class="row p-4">
            <table class="table mx-auto justify-content-center border" style="width: 80%">
                ...
                <tbody>
                <?php
                    while($row = $solicitudes_data->fetch_assoc()) {
                        echo '<tr class="clickable-row" data-href="detalle_solicitud.php?id='.$row["id"].'">';
                        echo '<th scope="row">'.$row["nombre_solicitante"].'</th>';
                        echo "<td>";
                        $especialidad = getEspecialidadSolicitante($db, $row["especialidad_id"]);
                        while($especialidad_row = $especialidad->fetch_assoc()){
            ...
            <ul class="pagination justify-content-center">
                <?php
                    if($page<2){
                        ...
                        echo '<li class="page-item"><a class="page-link" href="ver_solicitudes_atencion.php?page='.($page-1).'" tabindex="-1">Anterior</a></li>';
                    }
